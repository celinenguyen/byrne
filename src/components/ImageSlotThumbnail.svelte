<script lang="ts">
  ...
  let { slot, slide, onUpdate, popoverOpen = false, onpopoverOpenChange }: Props = $props();
...
{#if slot.hasContent}
  ...
  {#if slot.isUsed}
    ...
    <AddImagePopover slotIndex={slot.index} onimage={onUpdate} open={popoverOpen} onOpenChange={onpopoverOpenChange}>
      {#snippet trigger()}
