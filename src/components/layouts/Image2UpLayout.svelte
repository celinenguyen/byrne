...
<BaseLayout>
  <div class="grid grid-cols-2 gap-4 h-full p-6">
    <div class="flex flex-col">
      ...
      {#if capLeft}
        <div use:autoScale={{ minScale: 0.7 }} data-slot="text:leftCaption" class="mt-2 prose prose-sm max-w-none" style="font-family: var(--slide-font-caption); color: var(--slide-color-primary)"><MarkdownText text={capLeft} /></div>
      {/if}
    </div>
    <div class="flex flex-col">
      <SlideLayoutImageSlot src={imgRight} slotName="right" class="flex-1 min-h-0 rounded" />
      {#if capRight}
        <div use:autoScale={{ minScale: 0.7 }} data-slot="text:rightCaption" class="mt-2 prose prose-sm max-w-none" style="font-family: var(--slide-font-caption); color: var(--slide-color-primary)"><MarkdownText text={capRight} /></div>
      {/if}
    </div>
  </div>
</BaseLayout>
