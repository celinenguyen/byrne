...
<BaseLayout>
  <div class="grid grid-cols-2 gap-4 h-full p-6">
    <div class="flex flex-col">
      <div class="layout-image-left flex-1 min-h-0 flex items-center justify-center bg-gray-50 rounded">
        {#if imgLeft}
      ...
      {#if capLeft}
        <div class="layout-text-leftCaption mt-2 text-xs text-gray-500 prose prose-sm max-w-none">{@html capLeftHtml}</div>
      {/if}
    </div>
    <div class="flex flex-col">
      <div class="layout-image-right flex-1 min-h-0 flex items-center justify-center bg-gray-50 rounded">
        {#if imgRight}
      ...
      {#if capRight}
        <div class="layout-text-rightCaption mt-2 text-xs text-gray-500 prose prose-sm max-w-none">{@html capRightHtml}</div>
      {/if}
    </div>
  </div>
</BaseLayout>
...
<style>
  ...
  .layout-text-leftCaption {
  }
  .layout-text-rightCaption {
  }
</style>
